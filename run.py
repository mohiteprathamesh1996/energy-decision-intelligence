"""
CLI entry point — PuLP edition.
CBC is bundled with PuLP; no --solver flag needed.

Usage:
  python run.py                     # 14-day base case
  python run.py --mode scenarios    # base + all 9 scenarios
  python run.py --mode stochastic   # stochastic analysis (EVPI/VSS/VaR)
  python run.py --mode full         # everything
  python run.py --horizon 7         # shorter horizon
  python run.py --verbose           # show CBC solver log
"""
...
def _div(w=65): print("─" * w)


def run_base(horizon: int, verbose: bool):
    ...
    result = MultiPeriodOptimizer(net, verbose=verbose).solve()
    ...
    print()
    _div()
    print(f"  BASE CASE  |  {T}-day horizon  |  PuLP/CBC")
    _div()
    ...
    for c in net.contracts:
        avg_flow = sum(result.flows_by_period.get((c.arc_origin, c.arc_dest, t), 0)
                       for t in range(1, T + 1)) / T
        avg_def  = sum(result.sop_deficits.get((c.arc_origin, c.arc_dest, t), 0)
                       for t in range(1, T + 1)) / T
...
def run_scenarios(horizon: int, base_result, base_net):
    ...
    sc_results = runner.run_all(build_standard_scenarios(base_net))

    total_demand_base = sum(n.demand for n in base_net.get_nodes_by_type(NodeType.DEMAND))

    print()
    _div(85)
    print(f"  {'SCENARIO':<35} {'NET MARGIN':>12}  {'Δ BASE':>12}  {'SVC LEVEL':>9}  {'CARBON t/d':>10}")
    _div(85)
    for sr in sc_results:
        ...
        print(
            ...
            f"{svc:>8.1f}%  {carbon:>8.0f}"
        )
    _div(85)


def run_stochastic(horizon: int):
    from data.generate_data import build_base_network
    from src.model.stochastic import run_stochastic_analysis

    print("\n  Running two-stage stochastic analysis (10 scenarios)...")
    net = build_base_network(horizon=horizon)
    sr  = run_stochastic_analysis(net, n_scenarios=10, use_extensive_form=False)
    ...
    print(f"  VaR 95%                      : ${sr.risk.var_95:>12,.0f}")
    print(f"  CVaR 95%                     : ${sr.risk.cvar_95:>12,.0f}")
...
def main():
    p = argparse.ArgumentParser(description="Oil Supply Chain Optimizer — PuLP edition")
    p.add_argument("--mode",    choices=["base", "scenarios", "stochastic", "full"], default="base")
    p.add_argument("--horizon", type=int, default=14)
    p.add_argument("--verbose", action="store_true", help="Show CBC solver output")
    args = p.parse_args()

    result, net = run_base(args.horizon, args.verbose)

    if args.mode in ("scenarios", "full"):
        run_scenarios(args.horizon, result, net)

    if args.mode in ("stochastic", "full"):
        run_stochastic(args.horizon)
